...
def index(request):
    title = "Welcome to Shoppingado!!!"
    return render(request,'merchandise/index.html', {'title':title})
...
def featuredproductList(request):
    products = Product.objects.filter(featured_item=True)
    return render(request,'merchandise/featured_products.html',{'all_products':products})
